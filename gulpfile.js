...
var settings = 
{
	jsSrc: 'application/script/*.js',
	jsDest: 'htdocs/assets/js',
	sassWatch: 'application/style/*.scss',
	sassSrc: ['application/style/*.scss', '!_*.scss'],
	sassDest: 'htdocs/assets/css',
    sassConfig: {
	...
	downloadDest: 'application/download',
 ...
 */
 gulp.task('init', ['copy-assets', 'compile-sass', 'uglify-js']), function() {
...
{
    return gulp.src('application/download/*')
            .pipe(clean())
...
gulp.task('clean-assets', function() {
	return gulp.src(['htdocs/assets/*', '!htdocs/assets/upload'], {read: false})
		.pipe(clean())
		.pipe(notify('Deleted: <%= file.relative %>'));
});
...
gulp.task('cleanup', ['clean-assets', 'clean-download'], function() {
	return gulp.src(['htdocs/css/*.css', 'htdocs/js/*.js'], {read: false})
		.pipe(clean())
		.pipe(notify('Deleted: <%= file.relative %>'));
});
